    ...
    <table class="table table-border table-sm" width=”500″ height=”250″ style="font-size: 14px" border="2">
        ...
        <tbody>
           ...
           {{\Carbon\Carbon::parse($tunais->tanggal)->isoFormat('D MMMM Y')}}
            ...
            <td>
                {{$loop->iteration == 1? __('Tunai'):($tunais->angsuran)}}
            </td>
            <td>
                Rp.{{number_format(($loop->iteration == 1 ? $cash : $tunais->bayartunai), 0, ',','.')}}
            </td>
            <td>

            </td>
            <td>
                Rp.{{number_format(($loop->iteration == 1 ? $hargacash : $hargacash -= ($tunais->bayartunai)), 0, ',','.')}}</td> 
            </tr>    
            @endforeach 
